// src/index.js
import { mkdir as mkdir8, readFile as readFile4, readdir as readdir4, realpath, rename as rename8, rm as rm3, stat as stat4, unlink, writeFile as writeFile8 } from "node:fs/promises";
import { basename as basename3, dirname as dirname4, isAbsolute, join as join10 } from "node:path";
...
function createMoveNoticeStore(options = {}) {
  const dir = options.dir || process.env.DSH_SESSIONS_MANAGER_PENDING_DIR || process.env.DSH_SESSIONS_MANAGER_STAR_DIR || join3(homedir3(), ".dsh", "sessions-manager");
  const indexPath = options.indexPath || join3(dir, "move-notices.json");
  let mutation = Promise.resolve();
  function read() {
    try {
      return normalizeStore2(JSON.parse(readFileSync3(indexPath, "utf8")));
    } catch (e) {
  ...
  async function write(store) {
    ...
    const tmp = join3(dir, `.move-notices-${process.pid}-${Date.now()}.tmp`);
    await writeFile3(tmp, JSON.stringify(normalizeStore2(store), null, 2), { encoding: "utf8", mode: 384 });
...
var DEFAULT_DIR2 = join5(homedir4(), ".dsh", "sessions-manager");
...
function createAutoArchiveStore(options = {}) {
  ...
  const indexPath = options.indexPath || join5(dir, "auto-archive.json");
  let mutation = Promise.resolve();
  async function read() {
    try {
      return normalizeAutoArchiveStore(JSON.parse(readFileSync4(indexPath, "utf8")));
    } catch {
  ...
  async function write(store) {
    ...
    const tmp = join5(dir, `.auto-archive-${process.pid}-${Date.now()}.tmp`);
    await writeFile4(tmp, JSON.stringify(normalizeAutoArchiveStore(store), null, 2), { encoding: "utf8", mode: 384 });
...
import { mkdir as mkdir5, readFile, rename as rename5, writeFile as writeFile5 } from "node:fs/promises";
...
function createTitleIndexStore({ dir, file }) {
  ...
  const path = file || join6(dir, "title-index.json");
  ...
  return {
    ...
    async merge(batch) {
      ...
      await enqueue(async (store) => {
        ...
        const tmp = join6(dirname(path), `.title-index-${process.pid}-${Date.now()}.tmp`);
        await writeFile5(tmp, JSON.stringify({ schemaVersion: TITLE_INDEX_SCHEMA_VERSION, entries: next }), { encoding: "utf8", mode: 384 });
    ...
    async remove(ids) {
      ...
      await enqueue(async (store) => {
        ...
        await mkdir5(dirname(path), { recursive: true });
        const tmp = join6(dirname(path), `.title-index-${process.pid}-${Date.now()}.tmp`);
        await writeFile5(tmp, JSON.stringify({ schemaVersion: TITLE_INDEX_SCHEMA_VERSION, entries: store }), { encoding: "utf8", mode: 384 });
        await rename5(tmp, path);
      });
      return true;
    }
...
import { mkdir as mkdir6, readFile as readFile2, rename as rename6, writeFile as writeFile6 } from "node:fs/promises";
...
function createEmptyScanStore({ dir, file }) {
  let cache = null;
  let chain = Promise.resolve();
  const path = file || join7(dir, "empty-scan.json");
  async function readRaw() {
    try {
  ...
  return {
    ...
    async merge(batch) {
      ...
      await enqueue(async (store) => {
        ...
        const tmp = join7(dirname2(path), `.empty-scan-${process.pid}-${Date.now()}.tmp`);
        await writeFile6(tmp, JSON.stringify({ schemaVersion: EMPTY_SCAN_SCHEMA_VERSION, entries: next }), { encoding: "utf8", mode: 384 });
    ...
    async remove(ids) {
      ...
      await enqueue(async (store) => {
        ...
        await mkdir6(dirname2(path), { recursive: true });
        const tmp = join7(dirname2(path), `.empty-scan-${process.pid}-${Date.now()}.tmp`);
        await writeFile6(tmp, JSON.stringify({ schemaVersion: EMPTY_SCAN_SCHEMA_VERSION, entries: store }), { encoding: "utf8", mode: 384 });
        await rename6(tmp, path);
      });
      return true;
    }
...
function deriveSessionDir(root, cwd, id) {
  const project = cwd === void 0 || cwd === null || cwd === "" ? join8(root, "_no-cwd") : join8(root, projectKeyFor(cwd));
  return join8(project, encodeSegmentFor(id));
}
...
async function locateSessionArtifacts(sp, header) {
  ...
  const logPath = join8(sessionDir, generationFiles[0]);
  if (!pathOwnsSession(logPath, sid)) return null;
  return {
    root,
    projectDir: join8(root, header.cwd === void 0 || header.cwd === null || header.cwd === "" ? "_no-cwd" : projectKeyFor(header.cwd)),
    sessionDir,
    logPath,
    generationFiles,
...
import { mkdir as mkdir7, readFile as readFile3, readdir as readdir3, rename as rename7, rm as rm2, stat as stat3, writeFile as writeFile7 } from "node:fs/promises";
...
async function cleanSiblingCopies(root, sid, keepDir, keepVersion) {
  ...
  for (const ent of projects) {
    ...
    const candidate = join9(root, ent.name, segment);
...
async function relocateRewrittenBackup({ sid, canonical, backupLogPath, artifacts }) {
  ...
  const staged = join9(targetDir, `.move-stage-${process.pid}-${Date.now()}`);
  await writeFile7(staged, rewritten, { mode: 384 });
  await rename7(staged, join9(targetDir, artifacts.generationFiles[0]));
  return targetDir;
}
async function moveSessionToCwd({ sp, sid, header, canonical, events = [], inheritedEventCount = 0 }) {
  ...
  const backupRoot = join9(dirname3(artifacts.root), "sessions-manager-move-backup");
  const backupDir = join9(backupRoot, `${encodeSegmentFor(sid)}-${process.pid}-${Date.now()}`);
  await mkdir7(backupRoot, { recursive: true });
  const backupLogPath = join9(backupDir, basename2(artifacts.logPath));
  await rename7(artifacts.sessionDir, backupDir);
  let writer = null;
  try {
    try {
    ...
    try {
      await mkdir7(dirname3(artifacts.sessionDir), { recursive: true });
    } catch (_) {
    }
    try {
      await rename7(backupDir, artifacts.sessionDir);
    } catch (_) {
    }
...
// src/index.js
var BUILD_STAMP = true ? "3.6.2+ce2e87a5" : "dev";
var name = "dsh-sessions-manager";
var inject = ["webServer", "workspaceRegistry", "sessionPersistence", "sessionQuery", "storageDomain"];
var MAX_TITLE = 80;
var STATE_DIR = process.env.DSH_SESSIONS_MANAGER_STAR_DIR || join10(homedir5(), ".dsh", "sessions-manager");
var TRASH_DIR = process.env.DSH_SESSIONS_MANAGER_TRASH_DIR || join10(homedir5(), ".dsh", "sessions-manager-trash");
var TRASH_INDEX = join10(TRASH_DIR, "index.json");
...
function apply(ctx) {
  ...
  const titleIndex = createTitleIndexStore({ dir: TRASH_DIR, file: join10(TRASH_DIR, "title-index.json") });
  ...
  async function readTrashStore() {
    try {
      return normalizeTrashStore(JSON.parse(readFileSync5(TRASH_INDEX, "utf8")));
    } catch (e) {
  ...
  async function writeTrashStore(store) {
    await mkdir8(TRASH_DIR, { recursive: true });
    const tmp = join10(TRASH_DIR, `.index-${process.pid}-${Date.now()}.tmp`);
    await writeFile8(tmp, JSON.stringify(normalizeTrashStore(store), null, 2), { encoding: "utf8", mode: 384 });
    await rename8(tmp, TRASH_INDEX);
  }
  ...
  async function moveTargetWorkspace(rawPath) {
    ...
    if (p.startsWith("~/")) p = join10(homedir5(), p.slice(2));
    if (!isAbsolute(p)) p = join10(homedir5(), p);
    let canonical = null;
    try {
  ...
  async function moveOne(sid, targetPath) {
    ...
    const relocateLog = async (header, newHeaderObj) => {
      ...
      try {
        await mkdir8(dirname4(newPath), { recursive: true });
        try {
          await stat4(newPath);
        ...
        await rename8(oldPath, backupPath);
        ...
        await writeFile8(stagedPath, rewritten, { mode: 384 });
        await rename8(stagedPath, newPath);
        ...
        try {
          await rename8(backupPath, oldPath);
        } catch (_) {
        }
        if (e && e.code !== "ENOENT") throw e;
        return false;
