// src/index.js
import { mkdir as mkdir6, readFile as readFile3, readdir as readdir4, realpath, rename as rename6, rm as rm3, stat as stat4, unlink, writeFile as writeFile6 } from "node:fs/promises";
import { basename as basename3, dirname as dirname3, isAbsolute, join as join8 } from "node:path";
...
function deriveSessionDir(root, cwd, id) {
  const project = cwd === void 0 || cwd === null || cwd === "" ? join6(root, "_no-cwd") : join6(root, projectKeyFor(cwd));
  return join6(project, encodeSegmentFor(id));
}
...
async function locateSessionArtifacts(sp, header) {
  ...
  const logPath = join6(sessionDir, generationFiles[0]);
  if (!pathOwnsSession(logPath, sid)) return null;
  return {
    root,
    projectDir: join6(root, header.cwd === void 0 || header.cwd === null || header.cwd === "" ? "_no-cwd" : projectKeyFor(header.cwd)),
    sessionDir,
    logPath,
    generationFiles,
...
import { mkdir as mkdir5, readFile as readFile2, readdir as readdir3, rename as rename5, rm as rm2, stat as stat3, writeFile as writeFile5 } from "node:fs/promises";
...
async function cleanSiblingCopies(root, sid, keepDir, keepVersion) {
  ...
  for (const ent of projects) {
    ...
    const candidate = join7(root, ent.name, segment);
...
async function relocateRewrittenBackup({ sid, canonical, backupLogPath, artifacts }) {
  const original = await readFile2(backupLogPath);
  ...
  const staged = join7(targetDir, `.move-stage-${process.pid}-${Date.now()}`);
  await writeFile5(staged, rewritten, { mode: 384 });
  await rename5(staged, join7(targetDir, artifacts.generationFiles[0]));
  return targetDir;
}
async function moveSessionToCwd({ sp, sid, header, canonical, events = [], inheritedEventCount = 0 }) {
  ...
  const backupRoot = join7(dirname2(artifacts.root), "sessions-manager-move-backup");
  const backupDir = join7(backupRoot, `${encodeSegmentFor(sid)}-${process.pid}-${Date.now()}`);
  await mkdir5(backupRoot, { recursive: true });
  const backupLogPath = join7(backupDir, basename2(artifacts.logPath));
  await rename5(artifacts.sessionDir, backupDir);
  let writer = null;
  try {
    try {
    ...
    try {
      await mkdir5(dirname2(artifacts.sessionDir), { recursive: true });
    } catch (_) {
    }
    try {
      await rename5(backupDir, artifacts.sessionDir);
    } catch (_) {
    }
...
function classifyLineage(header, sizeBytes) {
...
// src/index.js
var BUILD_STAMP = true ? "3.5.4 built 2026-09-10T09:33:37.833Z" : "dev";
var name = "dsh-sessions-manager";
var inject = ["webServer", "workspaceRegistry", "sessionPersistence", "sessionQuery", "storageDomain"];
var MAX_TITLE = 80;
var STATE_DIR = process.env.DSH_SESSIONS_MANAGER_STAR_DIR || join8(homedir4(), ".dsh", "sessions-manager");
var TRASH_DIR = process.env.DSH_SESSIONS_MANAGER_TRASH_DIR || join8(homedir4(), ".dsh", "sessions-manager-trash");
var TRASH_INDEX = join8(TRASH_DIR, "index.json");
...
function apply(ctx) {
  ...
  const titleIndex = createTitleIndexStore({ dir: TRASH_DIR, file: join8(TRASH_DIR, "title-index.json") });
  ...
  async function writeTrashStore(store) {
    await mkdir6(TRASH_DIR, { recursive: true });
    const tmp = join8(TRASH_DIR, `.index-${process.pid}-${Date.now()}.tmp`);
    await writeFile6(tmp, JSON.stringify(normalizeTrashStore(store), null, 2), { encoding: "utf8", mode: 384 });
    await rename6(tmp, TRASH_INDEX);
  }
  ...
  async function moveTargetWorkspace(rawPath) {
    ...
    if (p.startsWith("~/")) p = join8(homedir4(), p.slice(2));
    if (!isAbsolute(p)) p = join8(homedir4(), p);
    let canonical = null;
    try {
  ...
  async function moveOne(sid, targetPath) {
    ...
    const relocateLog = async (header, newHeaderObj) => {
      ...
      try {
        await mkdir6(dirname3(newPath), { recursive: true });
        try {
          await stat4(newPath);
        ...
        await rename6(oldPath, backupPath);
        ...
        await writeFile6(stagedPath, rewritten, { mode: 384 });
        await rename6(stagedPath, newPath);
        destinationInstalled = true;
        await unlink(backupPath);
        const keptInSource = await cleanupMovedSourceDir(dirname3(oldPath));
        ...
        try {
          await rename6(backupPath, oldPath);
        } catch (_) {
        }
        if (e && e.code !== "ENOENT") throw e;
        return false;
