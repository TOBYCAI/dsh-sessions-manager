...
var MAX_TITLE = 80;
var TRASH_DIR = process.env.DSH_SESSIONS_MANAGER_TRASH_DIR || join(homedir(), ".dsh", "sessions-manager-trash");
var TRASH_INDEX = join(TRASH_DIR, "index.json");
...
function apply(ctx) {
  ...
  async function readTrashStore() {
    try {
      return normalizeTrashStore(JSON.parse(readFileSync(TRASH_INDEX, "utf8")));
    } catch (e) {
  ...
  async function writeTrashStore(store) {
    ...
    await writeFile(tmp, JSON.stringify(normalizeTrashStore(store), null, 2), { encoding: "utf8", mode: 384 });
    ...
    function findZstdFrameStarts(buf) {
      ...
      for (let i = 0; i + 4 <= buf.length; i++) {
        if (buf.readUInt32LE(i) === MAGIC) {
          try {
            zlib.zstdDecompressSync(buf.subarray(i, i + Math.min(buf.length - i, 1e6)));
            starts.push(i);
          } catch (_) {
          }
        }
