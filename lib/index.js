...
var MAX_TITLE = 80;
var TRASH_DIR = join(homedir(), ".dsh", "sessions-manager-trash");
...
function apply(ctx) {
  ...
  async function readTrash() {
    try {
      return JSON.parse(readFileSync(TRASH_INDEX, "utf8")) || [];
    } catch (e) {
      return [];
    }
  }
  ...
  async function deleteOne(sid) {
    ...
    const next = list.some((t) => t.sessionId === sid) ? list.map((t) => t.sessionId === sid ? entry : t) : list.concat(entry);
  ...
  async function allSessionItems() {
    ...
    let trashedIds = /* @__PURE__ */ new Set();
    try {
      trashedIds = new Set((await readTrash()).map((t) => String(t.sessionId)));
    } catch (e) {
    }
